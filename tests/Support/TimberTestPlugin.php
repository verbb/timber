...
final class TimberTestPlugin
{
    public static function ensureInstalledAndMigrated(): void
    {
        ...
        if (!$plugins->isPluginInstalled('timber')) {
            $plugins->installPlugin('timber');
        } elseif (!$plugins->isPluginEnabled('timber')) {
            $plugins->enablePlugin('timber');
        }

        $plugin = $plugins->getPlugin('timber');
        if (!$plugin) {
            throw new \RuntimeException('Timber plugin failed to load for integration tests.');
        }
